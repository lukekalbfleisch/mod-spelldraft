...
def report_summary(results, args, chain_source, dbc_path=None, overrides=0):
    lines = []
    add = lines.append
    total = len(results)
    ...
    add("")
    if dbc_path:
        spell_source = (f"`{os.path.relpath(dbc_path, MODULE_ROOT)}` with **{overrides} "
                        f"`spell_dbc` override(s) applied**"
                        if overrides else
                        f"`{os.path.relpath(dbc_path, MODULE_ROOT)}`, raw - no `spell_dbc` "
                        "overlay, so any scoping done in SQL is NOT reflected here")
        add(f"Spells: {spell_source}.")
        flags = " --apply-spell-dbc" if overrides else ""
        add("")
        add("Reproduce: `python3 tools/classify_talent_synergy.py"
            f"{flags} --out docs/TALENT_SYNERGY.md`.")
        add("")
...
def build_report(results, args, school_mask_auras, chain_source, family_names,
                 dbc_path=None, overrides=0):
    lines, add = report_summary(results, args, chain_source, dbc_path, overrides)
    ...
    if custom:
        spellmods = [r for r in custom
                     if any(e["verdict"] in ("spellmod-global", "spellmod-class-scoped", "custom-family")
                            for e in r["effects"])]
        # Non-passive rank spells: the DBC row is the effective spell and a learned
        # non-passive spell is never auto-cast, so its auras never apply - which is
        # exactly why such a chain classifies as ability-grant (see classify_chains).
        unpassive = [r for r in spellmods if r["label"] == "ability-grant"]
        # Family 0 with a mask set: IsAffected returns true before it reads the mask,
        # so the per-spell targeting the pack was authored for is inert.
        inert = [r for r in spellmods
                 if any(e["verdict"] == "spellmod-global" and any(e["mask"]) for e in r["effects"])]
        cross = [r for r in custom if r["label"] == "school-wide-aura"]
        add("## Custom EQ talent pack (TabID 0)")
        add("")
        add(f"{len(custom)} chains whose rank spells ship in `Spell.dbc` (`994001+`, `talent_dbc`")
        add(f"written by `30_eq_talent_pack.sql`). {len(cross)} classify as `school-wide-aura`;")
        add("the remainder grant an active ability, which is class-bound by nature.")
        add("")
        if unpassive:
            add(f"**Defect 1 - {len(unpassive)} of the {len(spellmods)} spellmod chains never take "
                "effect.** A learned spell's auras")
            add("are only applied automatically when the spell is passive: `Player::_addSpell`")
            ...
            add("applied. Fix: mark the row passive in `build_eq_talent_row`.")
            add("")
        if inert:
            ...
            add("mask, so each mod applies to *every* spell the character casts instead of the")
            add(f"single EQ spell it was aimed at. Fix: `row[SF_FAMILY] = {EQ_CUSTOM_FAMILY}`")
            add("(keeping the bit in `SF_FAMILYFLAGS`), which also keeps the talent cross-class")
            add(f"because the EQ spells are themselves family {EQ_CUSTOM_FAMILY}.")
            add("")
        if not unpassive and not inert:
            add("**Both of the defects found here are fixed** (2026-09-19, in")
            add("`build_client_patch.py`'s `build_eq_talent_row`), which is why the numbers above")
            add("no longer read the way they did: the rank spells now carry")
            add(f"`Attributes = 0x1d0` (`SPELL_ATTR0_PASSIVE` included) and `SpellFamilyName =")
            add(f"{EQ_CUSTOM_FAMILY}` with the classmask bit for the one EQ spell each was authored")
            add("for, so they auto-apply on learning *and* stay aimed at their own spell. The")
            add("rebuilt `Spell.dbc` was verified field-by-field against the previously deployed")
            add("one: 25 rows changed, only words 4 (`Attributes`) and 208 (`SpellFamilyName`),")
            add("row count unchanged. The 4 chains still labelled `ability-grant` grant an active")
            add("ability and are not spellmods - passive is the wrong shape for them.")
            add("")
        else:
            add("Both defect fixes change the shipped patch, so they need `build_client_patch.py`")
            add("re-run plus a new client patch and `dbc/Spell.dbc`.")
            add("")
...
def main():
    ...
    dbc_path = os.path.abspath(resolve_dbc(args.dbc))
