...
`tools/school_scope_tier1.py`): **126 talents / 404 spells**. Their class scoping is
...
Only 3 school-lever talents remain unconverted: their only op is `THREAT`, whose aura is
consumed as a multiplier while the spellmod is additive, so the magnitude needs a
conversion that has not been verified.
...
- **One-spell talents** (46, e.g. `Improved Fireball`). Their classmask is a single bit,
  ...
  their tooltips say.
- **Three `THREAT`-only talents.** Their aura is a multiplier while the spellmod is
...
remains — after both batches that is 3 `THREAT`-only talents, 48 single-spell ones and 117
