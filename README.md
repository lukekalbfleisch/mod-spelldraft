...
A second batch covers the damage-side talents that use an op *with* a per-school lever
(`data/sql/db-world/34_school_scope_tier1.sql`, generated by
`tools/school_scope_tier1.py`): **90 talents / 283 spells**. Their class scoping is
...
|---|---|
| `DAMAGE`, `DOT` | `MOD_DAMAGE_PERCENT_DONE` |
...
magnitudes transfer 1:1 because both sides use the same units. `THREAT` is **excluded**:
...
Garrote and Ambush" → Physical), a school named in it ("of your Arcane spells" → Arcane),
or an explicit "all spells" / "your abilities" → every school. That resolves all 90.

The other 39 school-lever talents are excluded because they are healing-side (36 —
`MOD_HEALING_DONE_PERCENT` takes no school, so "you heal more" would be universal and
needs its own decision) or because their only op is `THREAT` (3), whose aura is consumed
as a multiplier while the spellmod is additive.
...
- **One-spell talents** (46, e.g. `Improved Fireball`). Their classmask is a single bit,
  ...
  their tooltips say.
- **Healing-side talents** (36 across both tiers). `MOD_HEALING_DONE_PERCENT` takes no
  school, so they need their own decision about whether "you heal more" should be
  universal; they also need a separate balance pass.
- **One `THREAT`-only talent group** (3). Its aura is a multiplier while the spellmod is
  additive, so the magnitude needs a conversion that has not been verified.
- **Effect-slot talents** (117). `ALL_EFFECTS`/`EFFECT1-3` are spell-specific by
  construction and need re-authoring rather than re-plumbing.

`python3 tools/classify_talent_synergy.py --breakdown` lists the current state of what
remains — after both batches that is 39 school-lever talents, 48 single-spell ones and 117
...
generated SQL lists every affected talent as that batch's todo.
