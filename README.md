...
- **One-spell talents** (46, e.g. `Improved Fireball`). Their classmask is a single bit,
  ...
  their tooltips say.
- **Damage/crit/cost talents** (129). These are converted to a school-mask aura instead
  ("you deal more Fire damage"), so they need a per-talent school choice.
...
`python3 tools/classify_talent_synergy.py --breakdown` lists the current state of those
three groups; `--apply-spell-dbc` makes either report reflect what the server actually
loads. `tools/broaden_talent_scoping.py --check` verifies the SQL is applied.
